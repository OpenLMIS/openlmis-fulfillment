            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network erd_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s fulfillment -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
