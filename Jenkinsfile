...
pipeline {
    ...
    environment {
        ...
        COMPOSE_PROJECT_NAME = "fulfillment-${BRANCH_NAME}"
    }
    parameters {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                withCredentials([file(credentialsId: '8da5ba56-8ebb-4a6a-bdb5-43c9d0efb120', variable: 'ENV_FILE')]) {
                    ...
                    sh "docker tag openlmis/fulfillment:latest openlmis/fulfillment:${STAGING_VERSION}"
                    sh "docker push openlmis/fulfillment:${STAGING_VERSION}"
                }
            }
            post {
                success {
            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        sudo mkdir -p /var/www/html/erd-fulfillment
                        sudo chown -R $USER:$USER /var/www/html/erd-fulfillment
                        ...
                        && wget https://raw.githubusercontent.com/OpenLMIS/openlmis-fulfillment/master/docker-compose.erd-generation.yml -O docker-compose.yml \
                        && (/usr/local/bin/docker-compose up &) \
                        && sleep 90 \
                        && sudo rm /var/www/html/erd-fulfillment/* -rf \
                        && sudo rm -rf output \
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network ${COMPOSE_PROJECT_NAME}_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s fulfillment -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
                        && /usr/local/bin/docker-compose down --volumes \
                        && sudo chown -R $USER:$USER output \
                        && mv output/* /var/www/html/erd-fulfillment \
                        && rm erd-fulfillment.zip -f \
                        && pushd /var/www/html/erd-fulfillment \
                        && zip -r $WORKSPACE/erd/erd-fulfillment.zip . \
                        && popd \
                        && rmdir output \
                        && rm .env \
                        && rm docker-compose.yml
                    '''
                    archiveArtifacts artifacts: 'erd-fulfillment.zip'
                }
            }
            post {
                failure {
        ...
        stage('Push image') {
            ...
            steps {
                sh "docker tag openlmis/fulfillment:${STAGING_VERSION} openlmis/fulfillment:${VERSION}"
                sh "docker push openlmis/fulfillment:${VERSION}"
            }
            post {
                failure {
                    slackSend color: 'danger', message: "${env.JOB_NAME} - #${env.BUILD_NUMBER} ${env.STAGE_NAME} FAILED (<${env.BUILD_URL}|Open>)"
    ...
    post {
        ...
        success {
            build job: 'OpenLMIS-fulfillment-deploy-to-test', wait: false
        }
    }
}
